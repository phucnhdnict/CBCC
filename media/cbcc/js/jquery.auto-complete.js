...
var
    ...
    AutoCompleteFunction = function( self, options ) {
        ...
        var $input = $( self ).attr( 'autocomplete', 'off' ),
            ...
            $ul = ! settings.newList && rootjQuery.find( 'ul#ul_ac_'+$input.attr('id')+'.' + settings.list )[ 0 ] ?
                rootjQuery.find( 'ul#ul_ac_'+$input.attr('id')+'.' + settings.list ).eq( 0 ).bgiframe( settings.bgiframe ) :
                $('<ul id="ul_ac_'+$input.attr('id')+'"/>').appendTo('body').addClass( settings.list ).bgiframe( settings.bgiframe ).hide().data( 'ac-selfmade', TRUE );
...
//                	console.log(currentList[i]);
                    if ( currentList[i].value ) {
                        ...
                        var str = '';
                        $.each(currentList[i],function(keyContent,valContent){
                        	if(keyContent != 'value'){
                        		str += ' data-'+keyContent+'="'+valContent+'"';
                        	}
                        	
                        });                        
                        container.push(
                            settings.striped && striped ? '<li class="' + settings.striped + '" '+str+'>' : '<li '+ str+'>',
