...
(function($)
		...
		B.execute=function(g,h){
			...
			var j=$(this);var k={index:-1,options:null};
			var l="lang";
			var m=false;
			$.browser.chrome=/chrome/.test(navigator.userAgent.toLowerCase())
			if($.browser.chrome)$.browser.safari=false;
			if($.meta){g=$.extend({},options,j.data())}
			var n=$("<div/>");
			var o=$("<div/>");
			var p=$("<input/>");
			var q=$("<select/>");
			...
			var u={option:function(a){return $(q.get(0).options[a])},selected:function()
					{return q.find(":selected")},selectedIndex:function(a){
						if(a>-1)q.get(0).selectedIndex=a;
						return q.get(0).selectedIndex},size:function(a){
							...
							reset:function(){
								...
								var f=a-d;u.size(e-d);
								q.empty();
								for(var i=d;i<e;i++)q.append($(j.get(0).options[i]).clone().attr(l,i-1));
								if(e>g.maxMultiMatch)q.append(r);q.get(0).selectedIndex=f}};draw();
								var x=false;
								o.mouseover(function(){	x=true});
								o.mouseout(function(){ x=false});
								q.mouseover(function(){x=true});
								q.mouseout(function(){x=false});
								p.click(function(e){if(!m)enable(e,true);
								else disable(e,true)});
								p.blur(function(e){				
								...
								j.click(function(e){q.focus()});
								q.click(function(e){
									if(u.selectedIndex()<0)return;
									disable(e)
								});
								q.focus(function(e){p.focus()});
								q.blur(function(e){if(!x)disable(e,true)});
								...
								p.keydown(function(e){
									...
									switch(e.keyCode){
										case 9:disable(e);moveTab(j,e.shiftKey?-1:1);break;
										case 13:disable(e);j.focus();break;
										case 27:disable(e,true);j.focus();break;
										case 33:
											if(u.selectedIndex()-q.attr("size")>0){
												u.selectedIndex(u.selectedIndex()-=q.attr("size"))}
											else{u.selectedIndex(0)}synchronize();break;
										case 34:
											if(u.selectedIndex()+q.attr("size")<q.get(0).options.length-1){
												u.selectedIndex(u.selectedIndex()+=q.attr("size"))}
											else{u.selectedIndex(q.get(0).options.length-1)}synchronize();break;
										case 38:if(u.selectedIndex()>0){u.selectedIndex(u.selectedIndex()-1);synchronize()}break;
										case 40:
											if(u.selectedIndex()<q.get(0).options.length-1){u.selectedIndex(u.selectedIndex()+1);
											synchronize()}break;
										default:return true}
									return false});
				function draw(){
					...
					if($.browser.msie)n.css("z-index",99999);
					o.css({"position":"absolute","top":0,"left":0,"width":j.outerWidth(),
						"height":j.outerHeight(),"background-color":"#FFFFFF","opacity":"0.01"});
					p.attr("type","text");p.hide();p.height(j.outerHeight());
					p.css({"position":"absolute","top":0,"left":0,"margin":"0px","padding":"0px",
						"outline-style":"none","border-style":"solid","border-bottom-style":"none",
					...
					if($.browser.msie&&parseInt(jQuery.browser.version)<8){
						p.css("padding","0px");p.css("padding-left","3px");
						p.css("border-left-width","2px");p.css("border-top-width","3px")}
					else if($.browser.chrome){p.height(j.innerHeight());
					...
					else{p.css("padding-left",parseFloatPx(j.css("padding-left"))+3);
					p.css("padding-top",parseFloatPx(j.css("padding-top"))+1)}
					var c=parseFloatPx(j.css("padding-left"))+parseFloatPx(j.css("padding-right"))+
					parseFloatPx(j.css("border-left-width"))+parseFloatPx(j.css("border-left-width"))+23;
					...
					j.css("width",w);
					j.css("font-size","12px");
					q.hide();
					u.size(j.get(0).length);
					q.css({"position":"absolute","top":j.outerHeight(),"left":0,"width":d,
						"border":"1px solid #333","font-weight":"normal","font-size":"13px","padding":0,
						"background-color":j.css("background-color"),"text-transform":j.css("text-transform")});
					//var e=/^\d+$/.test(j.css("z-index"))?j.css("z-index"):1;
					var e=/^\d+$/.test(j.css("z-index"))?99999:99999;
					//if(g.zIndex&&/^\d+$/.test(g.zIndex))e=g.zIndex;o.css("z-index",(e).toString(10));
					if(g.zIndex&&/^\d+$/.test(g.zIndex))e=g.zIndex;o.css("z-index",99999);
//					p.css("z-index",(e+1).toString(10));
//					q.css("z-index",(e+2).toString(10));
					p.css("z-index",99999);
					q.css("z-index",99999);
					j.wrap(n);j.after(o);j.after(p);j.after(q)};
	...
	function synchronize(){
		if(u.selectedIndex()>-1&&!u.selected().get(0).disabled)p.val(q.find(":selected").text());
		else p.val(j.find(":selected").text())
	};
	function store(){
	...
	var y=null;var z;var A;
	function searching(){
		if(z==A){y=null;return
	}
	var a=0;z=A;q.hide();q.empty();var b=escapeRegExp(A);
	if(g.exactMatch)b="^"+b;
	if(g.wildcards){
	...
	for(var i=1;i<j.get(0).length&&a<g.maxMultiMatch;i++){
		if(A.length==0||A.test(j.get(0).options[i].text)){
			...
			if(j.data("index")==i)d.text(j.data("text"));q.append(d);a++
		}
	}
	if(a>=1){u.selectedIndex(0)}
	else if(a==0){q.append(t)}
	if(a>=g.maxMultiMatch){q.append(r)}u.size(a);
	q.show();
	y=null
	};
	...
	function register(d){
		var e=$[d]={};
		$.fn[d]=function(b){b=$.extend(e.defaults,b);
		var c=this.size();
		return this.each(function(a){e.execute.call(this,b,c-a)
			})};
			return e}}
)(jQuery);