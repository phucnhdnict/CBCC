	...
	jQuery.ajax({
		...
		success: function(data){
			...
			for(i=0; i<data.length; i++){
				xhtml+='<tr>';
				xhtml+='<td><a style="cursor:pointer" idhoso="'+data[i].id+'" class="btn_edit_hoso">'+data[i].e_name+'</a>  <span  idHoso="'+data[i].id+'" class="btn btn-warning btn-mini word_2c">Xuất 2C Viên chức</span></td>';
...
jQuery(document).ready(function($){
	...
	$('body').delegate('.word_2c', 'click', function(){
		idHoso  = $(this).attr('idHoso');
		if(idHoso>0){
			url= '<?php echo JUri::base(true);?>/index.php?option=com_thongke&view=thongke&format=raw&task=word_2c&idHoso='+idHoso;
