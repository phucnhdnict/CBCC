...
class Thongke_Model_Bcdaotaoboiduong extends JModelLegacy{
	...
	function countBaocao($donvi_id, $where=null, $tungay=null, $denngay=null, $join=null){
		$db = JFactory::getDbo();
// 		if($tungay!="" || $tungay != null) $condition_thoigian .=' and lh.ngaybatdau >=  STR_TO_DATE("'.$tungay.'","%d/%m/%Y")';
// 		if($denngay!="" || $denngay != null) $condition_thoigian .=' and lh.ngayketthuc<= STR_TO_DATE("'.$denngay.'","%d/%m/%Y")';
		...
		$donviloaitru = $this->getUnManageDonvi(JFactory::getUser()->id, 'com_thongke', 'treeview' ,'bcdaotaoboiduong');
		if($donviloaitru!='')
			$query.=' and dv.id NOT IN ('.$donviloaitru.')';
// 		echo $query;exit;
		$db->setQuery($query);
